...
- Dashboard: Passkey-Auth (Multi-Passkey; NordPass-Fix `requireUserVerification:false`), Belegliste, Detail mit Audit-Trail, Freigabe mit SKR04-Korrektur, **Entwurf löschen** (Hash wird frei, Duplikatschutz bleibt; seit BER-130 als `entwurf_verworfen` protokolliert — die verworfene Belegnummer bleibt erklärbar)
- **Belegdatum ist Pflicht für die Freigabe** (BER-129): ohne Datum schrieb der Export still `0101` (= 1. Januar, aus `new Date(null)`). Häufigste Ursache: nur die erste Seite fotografiert. Server lehnt mit 422 ab, das Formular sperrt den Knopf und nennt den Grund
